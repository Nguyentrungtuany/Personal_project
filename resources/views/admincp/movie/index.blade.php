    ...
    <div class="max-w-full mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered table-hover align-middle text-center" id="tablephim"
                    style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $cate->title }}</td>
                            <td>
                                <a href="{{route('add-episode', $cate->id)}}" class="btn btn-primary btn-sm">Thêm
                                    tập</a>
                                @foreach($cate->episode as $key => $epis)
                                {{-- <span class="badge bg-dark"><a href="{{route('episode.edit', $epis->id)}}"
                                        style="color: white">Tập {{ $key+1 }}</a></span> --}}
                                <a class="show_video" data-movie_video_id="{{$epis->movie_id}}"
                                    data-video_episode="{{$epis->episode}}" style="color: white;cursor: pointer">
                            ...
                            <td>
                                @foreach($cate->movie_category as $value)
                                <span class="badge bg-secondary">{{ $value->title }}</span>
                                @endforeach
                            </td>
                            <td>
                                <select id="{{$cate->id}}" class="thuocphim_choose">
                                    <option value="phimle" {{ $cate->thuocphim=='phimle'?'selected':'' }}>Phim lẻ
                                    </option>
                                    <option value="phimbo" {{ $cate->thuocphim=='phimbo'?'selected':'' }}>Phim bộ
                                    </option>
                                </select>
                            </td>
                            <td>
                            ...
                            <td>
                                ...
                                <select id="{{$cate->id}}" class="resolution_choose">
                                    ...
                                    <option value="{{ $k }}" {{ $cate->resolution == $k ? 'selected' : '' }}>{{
                                        $resolution }}</option>
                                    @endforeach
                                </select>
                            </td>
                            ...
                            <td>
                                <div class="d-flex flex-column gap-2 align-items-center justify-content-center py-2">
                                    <a href="{{ route('movie.edit', $cate->id) }}"
                                        class="btn btn-warning btn-sm w-100 fw-semibold text-dark">Sửa</a>
                                    {!! Form::open(['method'=>'DELETE','route' => ['movie.destroy',
                                    $cate->id],'onsubmit' => 'return confirm("Xóa hay ko?")']) !!}
